    ...
    public function rules(): array
    {
        return [
            'codigo' => [
                ...
                'min:2',
                'max:50',
                Rule::unique('categorias', 'codigo')->where(fn ($query) => $query->where('estado', true)->where('empresa_id', $this->empresaId())),
            ],
            'nombre' => [
                'required',
                'string',
                'min:2',
                'max:150',
                Rule::unique('categorias', 'nombre')->where(fn ($query) => $query->where('estado', true)->where('empresa_id', $this->empresaId())),
            ],
