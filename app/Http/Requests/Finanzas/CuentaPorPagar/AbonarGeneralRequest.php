...
class AbonarGeneralRequest extends BaseRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            'proveedor_id' => [
                'required',
                'integer',
                Rule::exists('proveedores', 'id')->where(fn ($query) => $query->where('empresa_id', $this->empresaId())->where('estado', true)),
            ],
            'metodo_pago_id' => [
                'required',
                'integer',
    ...
    public function messages(): array
    {
        return [
            ...
            'proveedor_id.exists' => 'El proveedor seleccionado no existe o no pertenece a esta empresa.',
            ...
            'monto.required' => 'El monto a abonar/pagar es obligatorio.',
