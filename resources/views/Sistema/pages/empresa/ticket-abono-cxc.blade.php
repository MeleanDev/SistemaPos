...
<head>
    ...
    <title>Recibo de Abono #{{ str_pad($abono->id, 6, '0', STR_PAD_LEFT) }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body onload="window.print()">
    <div class="ticket">
        <div class="header">
            ...
            <div class="bold" style="margin-top: 5px; font-size: 13px;">COMPROBANTE DE ABONO</div>
            <div style="font-size: 11px;">RECIBO N° #{{ str_pad($abono->id, 6, '0', STR_PAD_LEFT) }}</div>
        </div>
        ...
        <div class="info">
            ...
            <div class="info-row">
                <span>CLIENTE:</span>
                <span class="bold">{{ $cliente->nombre }}</span>
            </div>
            <div class="info-row">
                <span>DOCUMENTO:</span>
                <span class="bold">{{ $cliente->cedula }}</span>
            </div>
            @if($cliente->telefono)
            <div class="info-row">
                <span>TELÉFONO:</span>
        ...
        <div class="totals">
            ...
            <div class="total-row bold" style="margin-top: 4px; color: #b91c1c;">
                <span>DEUDA GLOBAL CLIENTE:</span>
                <span>${{ number_format($deuda_total_cliente_usd, 2) }}</span>
            </div>
            <div style="font-size: 10px; text-align: right;">
                (Equivalente: Bs. {{ number_format($deuda_total_cliente_bs, 2) }})
            </div>
        </div>

        <div class="footer">
            <p class="bold">¡GRACIAS POR SU PAGO!</p>
            <p>COMPROBANTE DE PAGO NO FISCAL</p>
        </div>
    </div>
</body>
</html>
